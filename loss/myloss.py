...
class ContrastLoss(nn.Module):
    ...
    def js_div(self, inputs1, inputs2, get_softmax=False):
        """
            Function that measures JS divergence between target and output logits:
            """
        KLDivLoss = nn.KLDivLoss(reduction='batchmean')
        if get_softmax:
            inputs1 = F.softmax(inputs1)
            inputs2 = F.softmax(inputs2)
        log_mean_output = ((inputs1 + inputs2) / 2).log()
        return (KLDivLoss(log_mean_output, inputs1) + KLDivLoss(log_mean_output, inputs2)) / 2
            ...
            for i in range(len(list_a)):
                for j in range(len(list_b)):
                    if i != j:
                        ...
                        total_distance += 0.1*self.js_div(list_a[i], list_b[j]) + self.distance_func(list_a[i], list_b[j])
                        M += 1
        else:
            total_distance = 0.
            M = 0
    ...
    def forward(self, model_output):
        ...
        # positive loss
        pos_loss = (self.compare_samples(samples[0], samples[0], exclude_same=True) + self.compare_samples(samples[1],
                                                                                                           samples[1],
                                                                                                           exclude_same=True)) / 2
