...
def my_config():
    ...
    total_epoch = 15  # total number of epochs for training the model
    ...
    result_dir = '/data/xieyiping/projectone/Physnet/Contrast-phys/result_COHFACE'  # TODO:store checkpoints and training recording
...
def my_main(_run, total_epoch, fs, T, S, lr, in_ch, result_dir, delta_t, K):
    ...
    train_list, test_list = COHFACE_LU_split()  # TODO: you should define your function to split your dataset for training and testing
    np.save(exp_dir + '/train_list.npy', train_list)
    np.save(exp_dir + '/test_list.npy', test_list)

    # define the dataloader
    # 训练数据中没有标签
    dataset = COHFACEdataset(train_list, T)  # please read the code about H5Dataset when preparing your dataset
    ...
    for e in range(total_epoch):
        ...
        for it in range(np.round(40 / (T / fs)).astype('int')):
