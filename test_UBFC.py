...
def my_config():
    ...
    train_exp_dir = '/data/xieyiping/projectone/Physnet/Contrast-phys/result/%d'%train_exp_num  # training experiment directory
    # # test cross-data
    # epo = 29  # the model checkpoint at epoch e
...
def my_main(_run, epo, train_exp_dir, device, time_interval):
    ...
    # test_list = ["/data/xieyiping/dataset/PURE_numpy/PURE_filter_crop_numpy_box/07-06.npz"]
    ...
    # train_exp_dir2 = '/data/xieyiping/projectone/Physnet/Contrast-phys/result_PURE/1'
    # model.load_state_dict(torch.load(train_exp_dir2+'/epoch%d.pt'% (epo), map_location=device)) # load weights to the model
    ...
    for h5_path in test_list:
        ...
        imgs = f['frame']
        meta_data = h5_path.replace('UBFC_filter_crop_numpy_box','UBFC_filter_meta_numpy')
        ...
        # np.save(pred_exp_dir+'/'+h5_path.split('/')[-1][:-4], results)   # h5_path.split('/')[-1][:-4] = 'subject1'
